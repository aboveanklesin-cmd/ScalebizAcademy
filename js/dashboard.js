...
function showPaymentPendingBanner() {
  ...
  banner.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;">
      <span>✅</span>
      <span><strong style="color:#C8F135;">Payment received!</strong> &nbsp;Your course access will be activated within a few hours. Refresh this page after that.</span>
    </div>
...
async function init() {
  ...
  if (urlParams.has('razorpay_payment_id') || urlParams.has('razorpay_payment_link_id') || urlParams.get('payment') === 'pending') {
  ...
  LEVELS.forEach((lvl, idx) => {
    ...
    const scoreData = passed.find(p => p.challenge_name.includes(lvl.name));
    const scorePct  = scoreData ? scoreData.score : null;
  ...
  if (nextTest) {
    ...
    activeCard.innerHTML = `
      <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;">
        ...
        <a href="/test.html?level=${nextTest.level}" class="active-btn">Take test →</a>
      </div>`;
  }
